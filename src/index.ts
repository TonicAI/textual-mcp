...
server.tool(
  ...
  {
    text: z.string().describe("The text to de-identify"),
  ...
  withLogging(logger, "redact_text", async ({ text, generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }) => {
    const result = await client.redactText(text, buildRedactOpts({ generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }));
    return {
      content: [
        {
          type: "text" as const,
          text: JSON.stringify(
            {
              ...
              entitiesFound: result.deIdentifyResults?.length ?? 0,
              entities: result.deIdentifyResults,
            },
            null,
            2
          ),
...
server.tool(
  ...
  {
    texts: z.array(z.string()).describe("Array of text strings to de-identify"),
    generatorConfig: generatorConfigSchema,
    generatorDefault: generatorDefaultSchema,
    generatorMetadata: generatorMetadataSchema,
    customEntities: customEntitiesSchema,
  ...
  withLogging(logger, "redact_bulk", async ({ texts, generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }) => {
    const result = await client.redactBulk(texts, buildRedactOpts({ generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }));
    const summary = result.bulkRedactionResults.map((r, i) => ({
      index: i,
      redactedText: r.redactedText,
      entitiesFound: r.deIdentifyResults?.length ?? 0,
    }));
    return {
...
server.tool(
  "redact_json",
  "Redact PII from a JSON string. Preserves JSON structure, only redacts text values.",
  {
  ...
  withLogging(logger, "redact_json", async ({ jsonText, generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }) => {
    const result = await client.redactJson(jsonText, buildRedactOpts({ generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }));
    return {
      content: [
        {
          type: "text" as const,
...
server.tool(
  ...
  {
    xmlText: z.string().describe("The XML string to de-identify"),
    generatorConfig: generatorConfigSchema,
    generatorDefault: generatorDefaultSchema,
    generatorMetadata: generatorMetadataSchema,
    customEntities: customEntitiesSchema,
  ...
  withLogging(logger, "redact_xml", async ({ xmlText, generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }) => {
    const result = await client.redactXml(xmlText, buildRedactOpts({ generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }));
    return {
      content: [
        {
          type: "text" as const,
...
server.tool(
  ...
  {
    htmlText: z.string().describe("The HTML string to de-identify"),
    generatorConfig: generatorConfigSchema,
    generatorDefault: generatorDefaultSchema,
    generatorMetadata: generatorMetadataSchema,
    customEntities: customEntitiesSchema,
  ...
  withLogging(logger, "redact_html", async ({ htmlText, generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }) => {
    const result = await client.redactHtml(htmlText, buildRedactOpts({ generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }));
    return {
      content: [
        {
          type: "text" as const,
...
Supported formats: PDF, docx, xlsx, PNG, JPG, JPEG, TIF/TIFF.`,
  ...
  withLogging(logger, "redact_file", async ({ filePath, outputPath, generatorConfig, generatorDefault, generatorMetadata, customEntities, labelBlockLists, labelAllowLists }) => {
    ...
    if (textTypes.includes(ext)) {
      return {
        content: [{ type: "text" as const, text: `Error: ${ext} files should be redacted using the text-based redaction tools (redact_text, redact_json, redact_xml, redact_html) which are faster and return inline results.` }],
        isError: true,
      };
    }

    const job = await client.startFileRedaction(filePath);
    ...
    return {
      content: [
        {
          type: "text" as const,
          text: `Redacted file saved to: ${outputPath} (${buffer.length} bytes)`,
        },
      ],
    };
  })
...
server.tool(
  ...
  "List all unattached file redaction jobs and their statuses. Optionally filter to jobs from a recent time window (e.g. '1h', '30m', '7d').",
  {
    from: z
      .string()
      .optional()
      .describe("Time window to look back, e.g. '1h', '30m', '7d'. If omitted, returns all jobs."),
  },
  ...
  withLogging(logger, "get_job_error_logs", async ({ jobId }) => {
    ...
    return {
      content: [
        {
          type: "text" as const,
          text: logs || "No error logs available for this job.",
        },
      ],
    };
  })
      ...
      content: [
        {
          type: "text" as const,
          text: JSON.stringify(
            { datasetId: dataset.id, name: dataset.name, message: "Dataset created. Use upload_file_to_dataset to add files." },
            null,
            2
          ),
        },
...
server.tool(
  ...
  withLogging(logger, "list_datasets", async () => {
    ...
    const summary = datasets.map((d: Dataset) => ({
      id: d.id,
      name: d.name,
...
server.tool(
  ...
  withLogging(logger, "scan_directory", async ({ directoryPath, maxDepth }) => {
    if (!fs.existsSync(directoryPath)) {
      return {
        content: [{ type: "text" as const, text: `Error: Directory not found: ${directoryPath}` }],
        isError: true,
      };
    }
...
Use scan_directory first to preview what will be processed. Use redact_text or redact_file on individual items first to tune your generatorConfig before running this on the full folder.`,
  {
    ...
    deidentifyNames: z
      ...
      .optional()
      .default(false)
      .describe("If true, also de-identify folder and file names by running them through Textual text redaction"),
    generatorConfig: generatorConfigSchema,
    generatorDefault: generatorDefaultSchema,
    generatorMetadata: generatorMetadataSchema,
    customEntities: customEntitiesSchema,
    ...
    fileExtensions: z
      .array(z.string())
      .optional()
      .describe("If provided, only process files with these extensions (e.g. ['.pdf', '.docx', '.txt']). Others are skipped."),
    skipPatterns: z
      .array(z.string())
      .optional()
      .describe("Glob-like patterns of files/folders to skip (e.g. ['node_modules', '.git', '*.log'])"),
  },
  withLogging(logger, "deidentify_folder", async ({
    ...
    if (!fs.existsSync(sourcePath)) {
      return {
        content: [{ type: "text" as const, text: `Error: Source directory not found: ${sourcePath}` }],
        isError: true,
      };
    }
    ...
    // Name de-identification cache to maintain consistency
    ...
    async function deidentifyName(name: string): Promise<string> {
      ...
      // Separate extension from name for files
      const ext = path.extname(name);
      const baseName = ext ? name.slice(0, -ext.length) : name;

      try {
        logger.info("name_redact_start", { tool: "deidentify_folder", name });
        const result = await client.redactText(baseName, {
          ...opts,
          generatorDefault: opts.generatorDefault || "Synthesis",
        });
        // If nothing was redacted, keep original
        const newBase =
          result.deIdentifyResults && result.deIdentifyResults.length > 0
            ? result.redactedText
            : baseName;
    ...
    logger.info("work_collected", {
      tool: "deidentify_folder",
      textFiles: textItems.length,
      binaryFiles: binaryItems.length,
      skipped: results.length,
    });

    // Phase 2: Process text files in parallel (semaphore in client limits concurrency)
    async function processTextItem(item: WorkItem) {
      ...
      try {
        ...
        if (item.ext === ".json") {
          result = await client.redactJson(content, opts);
        } else if (item.ext === ".html" || item.ext === ".htm") {
          result = await client.redactHtml(content, opts);
        } else if (item.ext === ".xml") {
          result = await client.redactXml(content, opts);
        } else {
          result = await client.redactText(content, opts);
        }
    ...
    // Wait for all text and binary processing to complete
